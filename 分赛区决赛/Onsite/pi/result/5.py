#!/usr/bin/env python3
"""
5.0 队列版：海康相机实时流 + Canny边缘检测 + 自动FPGA推理 + 机械臂分拣 + 美观Web前端
基于 4.2.py 重构：
  - FPGA发送与机械臂动作完全解耦：机械臂运动时仍可触发FPGA检测
  - 新增机械臂任务队列（arm_queue）：FPGA结果先入队，由独立worker线程顺序执行
  - 前端实时展示队列状态：OK=绿色方块, NG=红色方块
  - 手动触发不再受arm_busy限制
"""
...
# ===================== 用户可配置参数 =====================
# ---- 视觉检测参数 ----
CENTER_RANGE        = 50     # ★ 中心基准线左右各多少像素的检测范围（总宽度 = 2 × CENTER_RANGE）
EDGE_COUNT_THRESHOLD= 300    # ★ 中心区域白色像素个数阈值，超过此值触发FPGA
CANNY_LOW           = 50
CANNY_HIGH          = 150
...
<div class="container">
    ...
    <div class="main-grid">
        ...
        <div class="video-grid">
            ...
            <div class="card">
                ...
                <img src="/video_feed_edge" alt="边沿流">
            </div>
            <div class="card">
    ...
    <div class="footer">系统运行中 · 按 Ctrl+C 停止 · v5.0</div>
</div>
...
def main():
    ...
    try:
        while True:
            ...
            # 计算中心区域的白色像素个数
            half_width = w // 2
            x_start = max(0, half_width - CENTER_RANGE)
            x_end = min(w, half_width + CENTER_RANGE)
            center_region = edges[:, x_start:x_end]
            center_edge_count = int(np.count_nonzero(center_region))

            # 在边缘图上画中心基准线和检测范围标记
            # 画中心基准线（绿色）
            cv2.line(edge_bgr, (half_width, 0), (half_width, h-1), (0, 255, 0), 1)
            # 画检测范围左右边界（蓝色虚线示意，用半透明短线段表示）
            cv2.line(edge_bgr, (x_start, 0), (x_start, h-1), (255, 0, 0), 1)
            cv2.line(edge_bgr, (x_end, 0), (x_end, h-1), (255, 0, 0), 1)
            # 显示中心区域边缘点数和阈值
            cv2.putText(edge_bgr, f"Center={center_edge_count}/{EDGE_COUNT_THRESHOLD}",
                        (half_width+5, 20),
