...
void print_concise_result(const DetectionResult& result) {
    ...
    std::cout << COLOR_YELLOW << "│" << COLOR_RESET << "   处理时间: " << get_current_time() << std::endl;
            ...
            if (w > 0 && h > 0 && obj.prob <= 1) {
                ...
                switch(obj.class_id) {
                    ...
                    case 1:  // 脏污 - 红色
                        ...
                        {
                            int area = w * h;
                            info_text = std::to_string(area) + "px²";
                        }
                        break;
                    case 2:  // 褶皱 - 绿色
                ...
                if ((obj.class_id == 1 || obj.class_id == 2 || obj.class_id == 3) && !info_text.empty()) {
                    ...
                    // 在图片右上角显示总缺陷面积
                    std::string area_text = "总缺陷面积: " + std::to_string(total_area) + "px² (" + std::to_string(defect_count) + "个)";
