...
void show_usage(const std::string& program_name) {
    ...
    std::cout << "  -L, --log <path>        日志文件路径 (默认: 不输出日志)\n";
    ...
    std::cout << "  " << program_name << " -D ./images -L detection.log  # 输出日志到文件\n";
...
bool parse_arguments(int argc, char** argv, 
    ...
    while ((opt = getopt_long(argc, argv, "I:M:D:O:L:Fh", long_options, &option_index)) != -1) {
        switch (opt) {
            case 'I':
                img_path = optarg;
                has_input = true;
...
/**
 * @brief 记录检测结果到日志文件
 * 日志格式: file_name\tdefect_name\tdefect_order\tx1\ty1\tx2\ty2
 * 注意：正常类别（zheng_chang）不记录到日志
 * 
...
void log_detection_result(const std::string& filename, const DetectionResult& result) {
    ...
    if (first_write) {
        ...
        if (!log_stream.is_open()) {
            std::cerr << COLOR_RED << "[警告] 无法打开日志文件: " << g_log_file << COLOR_RESET << std::endl;
            return;
        }
        
        // 写入表头
        log_stream << "file_name\tdefect_name\tdefect_order\tx1\ty1\tx2\ty2\n";
        first_write = false;
        
        std::cout << COLOR_GREEN << "日志文件已创建: " << g_log_file << COLOR_RESET << std::endl;
    }
    ...
    int defect_order = 0;  // 缺陷序号，从1开始
    
    // 记录每个检测到的目标
    for (size_t i = 0; i < result.objects.size(); ++i) {
        const auto& obj = result.objects[i];
        
        // 检查是否为正常类别（zheng_chang），如果是则跳过记录
        if (obj.class_id == 4) {  // 正常类别
            continue;  // 跳过记录
        }
        
        // 获取类别名称
        std::string defect_name = "unknown";
        if (obj.class_id >= 0 && obj.class_id < static_cast<int>(CLASS_NAMES.size())) {
            defect_name = CLASS_NAMES[obj.class_id];
        }
...
void close_log_file() {
    if (!g_log_file.empty()) {
        // 如果有打开的文件流，会由析构函数自动关闭
        std::cout << COLOR_GREEN << "日志已保存到: " << g_log_file << COLOR_RESET << std::endl;
    }
}
...
void print_batch_summary(const std::vector<DetectionResult>& results) {
    ...
    if (!g_log_file.empty()) {
        std::cout << COLOR_BLUE << "│" << COLOR_RESET << "   日志文件: " << COLOR_BOLD << COLOR_GREEN << g_log_file << COLOR_RESET << std::endl;
    }
    if (total_images > 0) {
    ...
    for (int iw = 0; iw < count; iw++) {
        if (data[1] > thresh) {
            ...
            if (w > 0 && h > 0 && obj.prob <= 1) {
                ...
                // 绘制标签背景
                cv::rectangle(image, 
                            cv::Point(x-100, label_y - label_size.height - 5), 
...
bool process_single_image(const std::string& model_path,
    ...
    result.objects = visualize_result(
        outptr, static_cast<int>(cnt / 6), 0.3436f, img, CLASS_NAMES);
...
int main(int argc, char** argv) {
    ...
    if (!g_log_file.empty()) {
        std::cout << COLOR_YELLOW << "│" << COLOR_RESET << "   日志文件: " << COLOR_GREEN << g_log_file << COLOR_RESET << std::endl;
    }
