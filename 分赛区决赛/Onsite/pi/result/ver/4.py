    ...
    try:
        while True:
            ...
            if avg_x is not None:
                ...
                # 画avg_x竖线（红色）
                cv2.line(edge_bgr, (avg_x, 0), (avg_x, h_small-1), (0, 0, 255), 2)
                cv2.putText(edge_bgr, f"AvgX={avg_x}", (avg_x+5, 20),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
                # 画半宽线（绿色，作为触发阈值参考）
                cv2.line(edge_bgr, (half_width, 0), (half_width, h_small-1), (0, 255, 0), 1)
                cv2.putText(edge_bgr, f"Half={half_width}", (half_width+5, h_small-10),
