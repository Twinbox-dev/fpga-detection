...
std::shared_ptr<PaddlePredictor> LoadModel(std::string model_file,
  ...
  // config.set_model_file("./ssd_mobilenet_v1/__model__");
  // config.set_param_file("./ssd_mobilenet_v1/__params__");
...
void RunModel(std::map<std::string, std::string> config,
  ...
  auto rec_out = visualize_result(
      outptr, static_cast<int>(cnt / 6), 0.1f, img, class_names);
...
int main(int argc, char** argv) {
  ...
  int repeats = enable_benchmark ? 5: 1;
